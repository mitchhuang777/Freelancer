...
for element in tqdm(elements):
    ...
    if number:
        ...
        with warnings.catch_warnings():
            ...
            df = df.append({'Title': title, 'Number': number}, ignore_index=True)      
    else:
        # If the number was not found, save title to the excel file
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=FutureWarning)
            df = df.append({'Title': title, 'Number': np.nan}, ignore_index=True)
    df.to_excel("freelancer.xlsx", index=False)
"""
...
for job_listing in job_listings:
    ...
    avg_price = avg_price_element.text
    print(avg_price)
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=FutureWarning)
        df = df.append({'Project': project, 'Days': days, 'Avg Price': avg_price, 'Budget': budget, 'Bid': bid, 'URL': url, 'Paragraph': combined_text}, ignore_index=True)
    df.to_excel("freelancer.xlsx", index=False)
